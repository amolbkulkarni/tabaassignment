...
for(pc in 1:100)
{
  ...
  for (i in Reviews)
  {
    ...
    data.frame(
      ...
      Review_rating = length(html_nodes(i,'.icon-star-full-fill'))
    )
  }->Kia_Seltos_reviews_1
  write.table(Kia_Seltos_reviews_1, "f:\\isb\\taba\\Kia_Seltos_reviews3.csv",sep=",", row.names = FALSE, append = T)
}
...
for(pc in 1:100)
{
  ...
  write.table(Kia_Seltos_reviews_1, "f:\\isb\\taba\\jeep_compass_review.csv", sep = ",", row.names = FALSE, append = T)
}
...
for(pc in 1:100)
{
  ...
  write.table(Kia_Seltos_reviews_1, "f:\\isb\\taba\\mg_hector_review.csv", sep = ",", row.names = FALSE, append = T)
}
